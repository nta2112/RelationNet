...
class ResNet50Encoder(nn.Module):
    ...
    chỉ giữ phần trích xuất đặc trưng convolutional.

    Input:  (B, 3, 224, 224)  [hoặc bất kỳ kích thước nào >= 32]
    Output: (B, 2048, H, W)   [với 224x224 → H=W=7]
    """
    def __init__(self, pretrained=True):
        super(ResNet50Encoder, self).__init__()
        weights = models.ResNet50_Weights.IMAGENET1K_V1 if pretrained else None
        resnet  = models.resnet50(weights=weights)
        # Giữ tất cả các lớp trừ avgpool và fc
...
class RelationNetwork(nn.Module):
    """
    Mạng so sánh đặc trưng (Relation Network) – Adaptive.

    Dùng AdaptiveAvgPool2d nên hoạt động với mọi kích thước spatial:
      - Conv4:    ghép cặp → (B, 128,  19, 19)
      - ResNet50: ghép cặp → (B, 4096,  7,  7)

    in_channels : số channel của cặp ghép (= feat_channels * 2)
    hidden_size : số unit ẩn của lớp FC
    """
    ...
    def forward(self, x):
        ...
        out = self.gap(out).view(out.size(0), -1)  # (B, 64)
...
def main():
    ...
    if SPLIT_FILE is not None:
        # Format 1: split JSON chứa keys train/val/test
        ...
        # Format 2: mỗi split là một file JSON riêng (image-list)
        if DATA_ROOT is None:
        ...
        # Format 2 trả về dict → chuyển về list key để dùng với MiniImagenetTask
        # (task sẽ tự xử lý qua MiniImagenetTaskFromImageList)
        print(f"       meta-train: {len(metatrain_folders)} classes, "
              f"meta-test: {len(metatest_folders)} classes")

    else:
        # Cách gốc: quét thư mục
        print("[INFO] Dùng cách đọc gốc (quét thư mục)")
        metatrain_folders, metatest_folders = tg.mini_imagenet_folders()

    # Chọn đúng Task class theo loại dữ liệu
    ...
    if BACKBONE == "resnet50":
        ...
        feat_channels   = feature_encoder.out_channels  # 2048
        print(f"[INFO] ResNet50 pretrained={USE_PRETRAINED}, out_channels={feat_channels}")
    else:  # conv4
        feature_encoder = CNNEncoder()
        feat_channels   = feature_encoder.out_channels  # 64
        feature_encoder.apply(weights_init)

    # in_channels cho RelationNetwork = feat_channels * 2 (ghép cặp support+query)
    ...
    # Tên checkpoint bao gồm tên backbone để tránh nhầm lẫn
    ...
    for episode in range(EPISODE):
        ...
        sample_features = feature_encoder(samples.to(device))   # (N_s, C, H, W)
        batch_features  = feature_encoder(batches.to(device))   # (N_b, C, H, W)
        # Lấy kích thước spatial thực tế từ tensor (không hardcode)
